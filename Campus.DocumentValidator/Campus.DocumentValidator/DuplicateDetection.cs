...
namespace Campus.DocumentValidator
{
    ...
    internal class Detector
    {
        ...
        public DuplicateData GetResult()
        {
            ...
            foreach (DataKey each in Records.Values)
            {
                if (!each.IsDuplicated) continue;

                List<string> values = new JoinList<string>();
                List<int> positions = new JoinList<int>();

                foreach (KeyValuePair<string, string> eachKey in each.KeyData)
                    values.Add(eachKey.Value);

                foreach (int eachRow in each.Positions)
    ...
    internal class DataKey
    {
        ...
        public bool IsDuplicated { get { return Positions.Count > 1; } }
    }
